        ...
        private void OnCollisionStay(Collision col)
        {
            ...
            if (col.rigidbody != null)
            {
                ...
                if (otherAI != null || PlayerCar != null)
                {
                    ...
                    bool otherInFront = false; //Checking where the car is to prevent a null exception error

                    if (otherAI != null)
                    {
                        otherInFront |= Vector3.Angle(transform.forward, otherAI.transform.position - transform.position) < 90;
                    }

                    if (PlayerCar != null)
                    {
                        otherInFront |= Vector3.Angle(transform.forward, PlayerCar.transform.position - transform.position) < 90;
                    }

                    m_AvoidOtherCarSlowdown = otherInFront ? 0.5f : 1f;
